...
class CompliancePolicyRepository:
    ...
    def get_policy(self, profile: str, asset_class: str, office_id: str = "office_default") -> Optional[CompliancePolicy]:
        policies = self._load_policies()
        for p in policies.values():
            item = CompliancePolicy.from_dict(p)
            if item.office_id == office_id and item.profile.lower() == profile.lower() and item.asset_class.lower() == asset_class.lower():
                return item
        for p in policies.values():
            item = CompliancePolicy.from_dict(p)

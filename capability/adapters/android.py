...
class AndroidTTSAdapter(CapabilityAdapter):
    ...
    def speak(self, text: str, *, language: str = "pt-BR", pitch: float = 1.0, rate: float = 1.0) -> CapabilityResult:
        if not text.strip():
        ...
        if result.success:
            return CapabilityResult.ok({"spoken": True, "text": text[:80], "language": language}, self.name)
        return CapabilityResult.fail(
            result.stderr or "TTS failed",
            self.name,
...
class AndroidSMSAdapter(CapabilityAdapter):
    ...
    def send(self, number: str, message: str) -> CapabilityResult:
        ...
        if result.success:
            return CapabilityResult.ok({"sent": True, "to": number, "chars": len(message)}, self.name)
        return CapabilityResult.fail(
            result.stderr or "SMS send failed",
            self.name,
            reason="termux-sms-send returned non-zero",
            diagnosis=result.stderr,
            corrective_action=("pkg install termux-api && Settings → Apps → Termux → Permissions → SEND_SMS → Allow"),
        )

    def inbox(self, *, limit: int = 20, offset: int = 0) -> CapabilityResult:
        if not is_available("termux-sms-inbox"):
            return CapabilityResult.fail(
                "termux-sms-inbox not found", self.name, corrective_action="pkg install termux-api"
            )
        result = run(["termux-sms-inbox", "-l", str(limit), "-o", str(offset)], timeout=10)
        if not result.success:
            return CapabilityResult.fail(
                result.stderr or "inbox read failed",
                self.name,
                corrective_action=("Settings → Apps → Termux → Permissions → READ_SMS → Allow"),
            )
        try:
            messages = json.loads(result.stdout)
            return CapabilityResult.ok({"messages": messages, "count": len(messages)}, self.name)
        except json.JSONDecodeError as e:
            return CapabilityResult.fail(f"JSON parse error: {e}", self.name, diagnosis=result.stdout[:200])
...
class AndroidContactAdapter(CapabilityAdapter):
    ...
    def list_contacts(self) -> CapabilityResult:
        ...
        if not result.success:
            return CapabilityResult.fail(
                result.stderr or "contact list failed",
                self.name,
                corrective_action=(
                    "pkg install termux-api && Settings → Apps → Termux → Permissions → READ_CONTACTS → Allow"
                ),
            )
        try:
            contacts = json.loads(result.stdout)
            return CapabilityResult.ok({"contacts": contacts, "count": len(contacts)}, self.name)
        except json.JSONDecodeError as e:
            return CapabilityResult.fail(f"JSON parse error: {e}", self.name)
    ...
    def find(self, query: str) -> CapabilityResult:
        ...
        return CapabilityResult.ok({"contacts": matches, "count": len(matches), "query": query}, self.name)

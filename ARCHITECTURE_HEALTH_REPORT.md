...
**LLM Router addition** (`runtime/llm/`, added right after Sprint 25 as a priority directive): this is the strongest-tested new subsystem in the project by a real margin — 90+ tests across 11 files, unlike anything else here it mocks at the *actual network/subprocess boundary* per provider (`urllib.request.urlopen` for OpenRouter, `runtime.ollama`'s own functions for Ollama) rather than mocking the engine's own logic, and the Router itself is tested against real in-test `LLMProvider` implementations exercising genuine retry/fallback/cache/budget/metrics integration, not mock-calls-mock. The previously-flagged gap — `service.ask()` (Chat) not depending on the Router — is now closed: migrated in a follow-up pass that also added a provider-agnostic error taxonomy (`LLMAuthenticationError`/`LLMModelNotFoundError`/`LLMTimeoutError`) specifically so `cmd_ask`'s pre-existing differentiated error messages didn't regress, and hardened `LLMRouter` to preserve a single provider's specific error type instead of flattening it — a real bug this migration surfaced and fixed, not just a refactor. `runtime.ollama` no longer has any "generate a response" consumer outside `runtime/llm/providers/ollama_provider.py`; the remaining direct uses (`ping`/`models`/`doctor`/`/api/settings`/benchmarking) are deliberately out of scope — they introspect the local Ollama installation, not generate text, so the Router doesn't cover them.

**Sprint 25 addition** (`runtime/narrative/`): introduces the pipeline's first LLM dependency, reusing `runtime/ollama.py` rather than adding new LLM plumbing — zero new coupling. The one thing worth flagging for future tracking: this is also the pipeline's first component with a genuinely untestable-without-mocks external dependency (a live local model) — its 18 tests all mock `runtime.ollama`'s functions at the boundary, which is correct practice, but means the "does a real Ollama response actually get narrated correctly end-to-end" path has only been manually verified (this dev machine's Ollama consistently timed out well past 180s during live testing — an environment characteristic, not a code defect, but worth someone confirming on a machine with working local inference before relying on the "llm" source path in practice).

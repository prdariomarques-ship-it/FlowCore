...
**LLM Router addition** (`runtime/llm/`, added right after Sprint 25 as a priority directive): this is the strongest-tested new subsystem in the project by a real margin — 72 tests across 9 files, unlike anything else here it mocks at the *actual network/subprocess boundary* per provider (`urllib.request.urlopen` for OpenRouter, `runtime.ollama`'s own functions for Ollama) rather than mocking the engine's own logic, and the Router itself is tested against real in-test `LLMProvider` implementations exercising genuine retry/fallback/cache/budget/metrics integration, not mock-calls-mock. One deliberate, explicitly documented gap: `service.ask()` (Chat) was NOT migrated to depend on the Router, to avoid a real backward-compatibility break to its exception contract — this is the single clearest "do this next" item this report has flagged in a while, and unlike most items on this list it has a fully-specified fix already written down (`docs/LLM_ROUTER_ARCHITECTURE.md`'s "Known gap" section: migrate `ask()` and update 3 call sites' exception handling in the same change).

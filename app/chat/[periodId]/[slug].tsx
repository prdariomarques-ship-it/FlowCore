...
const suggestedQuestions = [
  ...
  "Por qual obra devo começar?",
  "Como seu contexto influenciou sua teologia?",
  "Como você responderia a uma objeção atual?",
];
...
export default function ChatScreen() {
      ...
      ? [
          {
            id: "welcome",
            role: "assistant",
            content: `A paz. Sou ${theologian.name}. Pode me apresentar uma questão; procurarei respondê-la a partir das preocupações e convicções do meu tempo.`,
          },
        ]
      : [],
  );
  ...
  const sendMessage = async (suggestion?: string) => {
    const trimmed = (suggestion ?? input).trim();
    ...
    <ScreenContainer edges={["top", "left", "right"]} containerClassName="bg-background">
      ...
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <View className="border-b border-border bg-surface px-5 pb-4 pt-2">
          <View className="flex-row items-center">
            ...
            <View className="mr-3 h-11 w-11 items-center justify-center rounded-full bg-primary">
              <Text className="font-serif text-lg font-bold text-background">{initials}</Text>
            </View>
            ...
            >
              <View className="rounded-full border border-border px-3 py-2">
                <Text className="text-[10px] font-bold uppercase tracking-widest text-primary">Nova</Text>
              </View>
            </Pressable>
          </View>
        </View>
        ...
        <FlatList
          ...
          ListHeaderComponent={
            messages.length === 1 ? (
              <View className="mb-5 rounded-2xl border border-primary/40 bg-primary/10 p-4">
                <Text className="text-xs font-bold uppercase tracking-widest text-primary">Ponto de partida</Text>
                <Text className="mt-2 text-sm leading-5 text-muted">{theologian.summary}</Text>
                <Text className="mt-4 text-xs font-bold uppercase tracking-widest text-foreground">Perguntas sugeridas</Text>
                  ...
                  {suggestedQuestions.map((question) => (
                    ...
                    >
                      <View className="rounded-xl border border-border bg-surface px-3 py-3">
                        <Text className="text-sm leading-5 text-foreground">{question}</Text>
                      </View>
                    </Pressable>
                  ))}
                </View>
          ...
          ListFooterComponent={
            <View>
              {isLoading && (
                <View className="mb-4 self-start rounded-2xl rounded-bl-sm border border-border bg-surface px-4 py-3">
                  <ActivityIndicator color="#C99A4A" />
                  <Text className="mt-2 text-xs text-muted">Consultando as obras e o contexto...</Text>
                </View>
              )}
              {error && (
                <View className="mb-3 rounded-xl border border-terracotta/60 bg-terracotta/10 p-3">
                  <Text className="text-sm leading-5 text-foreground">{error}</Text>
                  <Pressable onPress={() => setError(null)} style={({ pressed }) => ({ opacity: pressed ? 0.6 : 1 })}>
                    <Text className="mt-2 text-xs font-bold uppercase tracking-widest text-primary">Fechar aviso</Text>
                  </Pressable>
                </View>
              )}
            </View>
        ...
        <View className="border-t border-border bg-background px-4 pb-3 pt-3">
          <View className="flex-row items-end rounded-2xl border border-border bg-surface px-3 py-2">
            <TextInput
              ...
              multiline
              maxLength={1200}
              className="max-h-28 min-h-10 flex-1 px-2 py-2 text-[15px] leading-5 text-foreground"
              returnKeyType="send"
            ...
            <Pressable
              ...
              style={({ pressed }) => [{ opacity: !input.trim() || isLoading ? 0.35 : pressed ? 0.65 : 1 }]}
            >

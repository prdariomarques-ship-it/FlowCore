...
ALERT_DEFAULTS: dict[str, dict] = {
    "vix_absolute": {
        ...
        "threshold": 25.0,
        "severity": "warning",
        "label": "VIX acima de 25 (risco elevado)",
    },
    "vix_critical": {
        "source": "vix",
        "kind": "above",
        "threshold": 35.0,
        "severity": "critical",
        "label": "VIX acima de 35 (estresse severo)",
    },
    "treasury_10y_above": {
        "source": "treasury",
        "kind": "above",
    ...
    "usdbrl_above": {
        ...
        "threshold": 6.0,
        "severity": "warning",
        "label": "Dólar acima de R$ 6,00",
    },
    "sp500_daily_drop": {
        "source": "sp500",
        "kind": "daily_delta_below",
        "threshold": -3.0,
        "severity": "critical",
...
def evaluate_alerts(db_path: str | None = None) -> list[dict]:
    ...
    fired: list[dict] = []
    seen = set(row[0] for row in conn.execute("SELECT rule FROM market_alerts WHERE fired_at >= ?", (cutoff,)))
    for rule_name, rule in ALERT_DEFAULTS.items():
        ...
        conn.execute(
            "INSERT INTO market_alerts (rule, source, severity, label, payload, fired_at) VALUES (?,?,?,?,?,?)",
            (
                alert["rule"],
                alert["source"],
                alert["severity"],
...
def list_alerts(db_path: str | None = None, limit: int = 50) -> list[dict]:
    conn = _connect(db_path or _default_db_path())
    rows = conn.execute(
        "SELECT rule, source, severity, label, payload, fired_at FROM market_alerts ORDER BY fired_at DESC LIMIT ?",
        (limit,),
    ).fetchall()
    conn.close()
    out: list[dict] = []

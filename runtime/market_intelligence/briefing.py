...
def build_briefing() -> dict:
    ...
    lines: list[str] = []
    lines.append(
        f"CURVA EUA: {curve.state}" + (f" (10Y-2Y {curve.slope_10y_2y} bps)" if curve.slope_10y_2y is not None else "")
    )
    if curve.shape:
        lines.append(f"  formato: {curve.shape}" + (f" — {curve.interpretation}" if curve.interpretation else ""))
    ...
    for pair in fx["pairs"]:
        if pair["level"] is not None:
            lines.append(
                f"  {pair['name']}: {pair['level']:.4f} ({fmt(pair['delta_pct_1d'])}%) [USD {pair['usd_regime']}]"
            )
    for cls_name, cls in classes["classes"].items():
        movers = [s for s in cls["sources"] if s.get("delta") is not None and abs(s["delta"] or 0) >= 1]
        if movers:
            hl = ", ".join("{} {:+.1f}{}".format(s["source"], s["delta"], s["delta_unit"]) for s in movers)
    ...
    try:
        ...
        for dim, view in hist.get("windows", {}).items():
            ...
            if latest and latest.get("value") is not None:
                d5v = d5.get("value")
                trend = "estável" if d5v is None else ("subindo" if latest["value"] > d5v else "caindo")
                lines.append(
                    f"MACRO {dim.upper()}: {latest['value']:+.2f} ({latest['status']}) | tendência D-5: {trend}"
                )
    ...
    try:
        ...
        n = len(news["items"])
        lines.append(f"NOTÍCIAS: {n} itens recentes | " + (f"categorias: {', '.join(cats)}" if cats else "sem itens"))
